...
export default function TreePath() {
  ...
  const { setShowNew, showNew, reloadfetch ,showAdd ,setShowAdd} = useStateContext();
...
//   }
  ...
  const ContextMenu = ({ position, onDelete, nodeData }) => (
    <div className="context-menu fixed bg-mainLightBg shadow-2xl rounded-3xl p-7 " style={{ top: position.y, left: position.x }}>
      ...
      <ul className='flex flex-col gap-3 '>
        ...
        <li  className='cursor-pointer text-xl font-medium text-primary flex justify-center items-center bg-white hover:bg-secLightBg transition-colors  rounded-xl p-3 gap-3' onClick={() => AddChildRoute(nodeData)}><AiOutlineSubnode className='text-2xl'/>Ajouter un enfant</li>
        }
        ...
        <li className='cursor-pointer text-xl font-medium text-red-600 flex justify-center  items-center gap-3 bg-white hover:bg-secLightBg transition-colors  rounded-xl p-3'  onClick={onDelete}><MdOutlineDeleteForever className='text-2xl' /> Supprimer</li>
      </ul>
    </div>
  );
  ...
  return (
    <>
      ...
      {showAdd && <AddChild route={childData} />}
      {loading ? (
        <LoadingScreen />
      ) : (
