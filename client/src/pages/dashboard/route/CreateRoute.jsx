...
const CreateRoute = ({ routeId, parrentId }) => {
  ...
  const fetchRoutes = async () => {
    try {
      ...
      if (routeId) {
        ...
        const dataWithParrent = await axios.get(
          `${import.meta.env.VITE_API_URL}/route/${routeId?.parrentPath}`
        );
        ...
        const dataFields = Object.entries(routeId?.data?.tableCol || {}).map(
          ([tableCol, value]) => ({ tableCol, value })
        );
        setDataFields(dataFields);
  ...
  const handleUpdateRoute = async () => {
    try {
      ...
      error_toast(
        `Échec de la mise à jour de route: ${
          error.response ? error.response.data.message : error.message
        }`
      );
      console.error(error);
      ...
      >
        ...
        <div className="bg-white dark:bg-gray-800 mx-3">
          ...
          ${
            selectedRoute.view === option.name
              ? "border-textSecLightColor shadow-xl"
              : "border-transparent"
          }`}
                >
                  <img
          ...
          {selectedRoute.view === "PdfReader" && (
            <div className="my-4">
              ...
              <input
                ...
                className="hidden"
              />
              <p className="my-4 text-textSecLightColor font-semibold text-xl">
                {selectedRoute?.file}
              </p>
            </div>
          )}
          ...
          <div className="my-4">
            {selectedRoute.createdAt && (
              <p className="py-3  text-md font-medium text-gray-700 dark:text-gray-300 ">
                Date de création:{" "}
                {new Date(selectedRoute.createdAt).toLocaleDateString("fr-FR")}
              </p>
            )}
          </div>
          ...
          <div className="my-4">
            {parrentData?.data && (
              <>
                ...
                {parrentData?.data &&
                  Object.keys(parrentData?.data.tableCol).map((key, index) => (
                    <input
                      key={index}
                      label={`${key}`}
                      placeholder={key}
                      ...
                      onChange={(event) =>
                        handleFieldRowChange(index, event, key)
                      }
          ...
          {selectedRoute.view === "TableView" && (
            <div className="my-4">
              ...
              {dataFields.map((field, index) => (
                <div key={index} className="flex items-center space-x-4 my-6">
                  <input
                    ...
                    onChange={(event) =>
                      handleFieldTableChange(index, event, "tableCol")
                    }
                    className="px-3 py-3 mt-1 block w-full border-gray-300 rounded-md shadow-md dark:bg-gray-700 dark:text-white"
                  />
                  <div
              ...
              <div
                ...
                className="mt-4 w-fit text-white rounded"
              >
        ...
        <div className="my-6 mx-5 flex justify-end gap-7 ">
          ...
          <div onClick={routeId ? handleUpdateRoute : handleCreateRoute}>
            <Button Text={routeId ? "Mise à jour" : "Créer"} />
          </div>
        </div>
      </Dialog>
    </>
