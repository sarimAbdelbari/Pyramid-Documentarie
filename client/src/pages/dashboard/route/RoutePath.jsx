...
const RoutePath = () => {
  ...
  const [selectedRoute, setSelectedRoute] = useState({ groupName: '', path: '', permissions: [] });
  ...
  const fetchRoutes = async () => {
    try {
      ...
      const dataWithId = data.map((route, index) => ({ ...route, id: index + 1 }));
  ...
  const handleCreateRoute = async () => {
    try {
      const { data } = await axios.post('http://localhost:5000/api/route', selectedRoute);
      sucess_toast('Route created successfully');
      setRoutesData(prevState => [...prevState, { ...data, id: prevState.length + 1 }]);
      handleCloseDialog();
    } catch (error) {
      error_toast('Failed to create route');
      console.error(error);
    }
  };
  ...
  const handleUpdateRoute = async () => {
    try {
      const { data } = await axios.patch(`http://localhost:5000/api/route/${selectedRoute._id}`, selectedRoute);
      sucess_toast('Route updated successfully');
      setRoutesData(prevState => prevState.map(route => (route._id === data._id ? { ...data, id: route.id } : route)));
      handleCloseDialog();
    } catch (error) {
      error_toast('Failed to update route');
      console.error(error);
    }
  };
  ...
  const handleDeleteRoute = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/routes/${routeToDelete._id}`);
      sucess_toast('Route deleted successfully');
      setRoutesData(prevState => prevState.filter(route => route._id !== routeToDelete._id));
      handleCloseConfirmDialog();
    } catch (error) {
      error_toast('Failed to delete route');
      console.error(error);
    }
  };
  ...
  const handleOpenDialog = (type, route = { groupName: '', path: '', permissions: [] }) => {
    setDialogType(type);
  ...
  const handleCloseDialog = () => {
    setOpenDialog(false);
    setSelectedRoute({ groupName: '', path: '', permissions: [] });
  };
  ...
  return  (
    <div className='min-h-screen bg-mainLightBg dark:bg-mainDarkBg text-textLightColor '>
      ...
      <div className='pt-28 flex flex-col items-center gap-7'>
        <h3 className='text-3xl font-semibold text-textLightColor dark:text-textDarkColor leading-relaxed'>Routes</h3>
        <Button 
          variant="contained" 
            ...
            sx={{
              '& .MuiDataGrid-row': {
                ...
                '&:hover:nth-of-type(even)': {
                  backgroundColor: (theme) => theme.palette.mode === 'dark' ? '#2d3748' : '#e2e8f0',
                },
                '&:hover': {
                  backgroundColor: (theme) => theme.palette.mode === 'dark' ? '#2d3748' : '#e2e8f0',
                },
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent className='bg-secLightBg '>
          <TextField
            ...
            type="text"
            fullWidth
            value={selectedRoute.groupName}
            onChange={(e) => setSelectedRoute({ ...selectedRoute, groupName: e.target.value })}
          ...
          <Select
            multiple
            value={selectedRoute.permissions}
            onChange={(e) => setSelectedRoute({ ...selectedRoute, permissions: e.target.value })}
            renderValue={(selected) => selected.join(', ')}
            className='bg-secLightBg '
            style={{ marginTop: '1rem', width: '100%' }}
          >
            {['show', 'no-show', 'read', 'download'].map(permission => (
        ...
        <DialogActions className='bg-secLightBg '>
          ...
          <Button onClick={dialogType === 'create' ? handleCreateRoute : handleUpdateRoute} className='text-primary hover:text-darkPrimary'>
      ...
      <Dialog open={openConfirmDialog} onClose={handleCloseConfirmDialog}>
        ...
        <DialogContent className='bg-secLightBg '>
          Are you sure you want to delete this route ({routeToDelete?.groupName})?
        </DialogContent>
        <DialogActions className='bg-secLightBg '>
          <Button onClick={handleCloseConfirmDialog} className='text-primary hover:text-darkPrimary'>
            Cancel
          </Button>
          <Button onClick={handleDeleteRoute} className='text-primary hover:text-darkPrimary'>
            Delete
          </Button>
        </DialogActions>
      </Dialog>
